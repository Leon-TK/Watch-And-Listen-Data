...
namespace WAL::BufferHandlers
{
	std::vector<uint8_t> FileBufferHandler::ExtractData(size_t count)
	{
	...
	std::streampos FileBufferHandler::TellEofPos()
	{
	...
	FileBufferHandler::FileBufferHandler(std::ifstream* strm) : strm(strm)
	{

	}

	bool FileBufferHandler::canGetBeforeEnd(size_t count, size_t& outRemaining)
	{
	...
	size_t FileBufferHandler::GetFileSize()
	{
		if (!strm) return 0;
		return this->TellEofPos();
	}

	std::vector<uint8_t> FileBufferHandler::GetDataBytes(size_t count, bool doReturnPointer, bool& outIsComplete)
	{
		if (!strm) return std::vector<uint8_t>(0);
